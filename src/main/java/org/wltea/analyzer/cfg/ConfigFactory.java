...
public class ConfigFactory {
	...
	public static Configuration getDistributedConfig(Configuration configuration) {
		String key=configuration.isUseSmart()+":"+configuration.getDistributedDicPath();
		if(confMap.containsKey(key)) {
			return confMap.get(key);
		}
		Configuration conf=new MutableConfig(configuration.getDistributedDicPath());
		confMap.put(key, conf);
		return conf; 
	}
	/**
