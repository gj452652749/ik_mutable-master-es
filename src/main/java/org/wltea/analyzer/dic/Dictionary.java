...
public class Dictionary {
	...
	private void loadMainDict(){
		...
		//读取主词典文件
        InputStream is = this.getClass().getClassLoader().getResourceAsStream(cfg.getLocalDic());
        if(is == null){
	 ...
	 * 加载用户扩展的停止词词典
	 */
	private void loadStopWordDic(){
		//建立一个主词典实例
		_StopWordDict = new DictSegment((char)0);
		//加载扩展停止词典
		List<String> extStopWordDictFiles  = cfg.getExtStopWordDic();
	...
	private void loadQuantifierDic(){
		...
		//读取量词词典文件
        InputStream is = this.getClass().getClassLoader().getResourceAsStream(cfg.getQuantifierDic());
        if(is == null){
